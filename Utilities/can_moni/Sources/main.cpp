...
//    char *script_file = NULL;
    ...
    while ((optind = getOption(argc, (char**)argv, MAX_OPTIONS, option)) != EOF) {
        ...
        case OP_MODE_CHR:
            ...
            if (!_strcmpi(optarg, "DEFAULT") || !_strcmpi(optarg, "CLASSIC") || !_strcmpi(optarg, "CLASSICAL") ||
...
int CCanDevice::ListCanBitrates(CANAPI_OpMode_t opMode) {
    ...
    if (opMode.fdoe) {
        if (opMode.brse) {
            ...
            KVASER_CAN_FD_BR_1M8M(bitrate[0]);
            KVASER_CAN_FD_BR_500K4M(bitrate[1]);
            KVASER_CAN_FD_BR_250K2M(bitrate[2]);
            KVASER_CAN_FD_BR_125K1M(bitrate[3]);
            hasDataPhase = true;
            hasNoSamp = false;
            n = 4;
        }
            ...
            KVASER_CAN_FD_BR_1M(bitrate[0]);
            KVASER_CAN_FD_BR_500K(bitrate[1]);
            KVASER_CAN_FD_BR_250K(bitrate[2]);
            KVASER_CAN_FD_BR_125K(bitrate[3]);
            hasDataPhase = false;
            hasNoSamp = false;
            n = 4;
        }
        ...
        KVASER_CAN_BR_1M(bitrate[0]);
        KVASER_CAN_BR_800K(bitrate[1]);
        KVASER_CAN_BR_500K(bitrate[2]);
        KVASER_CAN_BR_250K(bitrate[3]);
        KVASER_CAN_BR_125K(bitrate[4]);
        KVASER_CAN_BR_100K(bitrate[5]);
        KVASER_CAN_BR_50K(bitrate[6]);
        KVASER_CAN_BR_20K(bitrate[7]);
        KVASER_CAN_BR_10K(bitrate[8]);
        KVASER_CAN_BR_5K(bitrate[9]);
        hasDataPhase = false;
        hasNoSamp = true;
        n = 10;
    }
